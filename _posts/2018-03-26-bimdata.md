...
the use of plug-ins. While IFC viewers are typically written as desktop applications using OpenGL or DirectX, writing them
 on WebGL gives us the advantage of being able to view them on the web. 
 <br><br>
 With IFC visualization, the challenge is how to engineer a WebGL-based viewer so as to be able to render large quantities 
 of objects within the memory and execution constraints of the browser, since models typically contain very large quantities of 
 elements (think: an individual element for every single fire sprinkler, electrical fitting, light panel etc!).
